...
#' fig_region_cea_km2()
fig_region_cea_km2_ceascf <- function(lang = "fr") {
  # output
  if (lang == "fr") {
  ...
  temp <- function(data_id, output_name, suffix = NULL, language = lang) {
    ...
    # Data
    # =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
    cekm <- read.csv(here::here("data", "ceascf", "data-output", glue::glue("{data_id}.csv")))
    ...
    for (i in 1:nGroup) {
      if (grNames$size[i] > 1) {
        ...
        text(
          x = -4, y = mean(grRange), labels = gsub(" ", "\n", grNames$title[i]),
  ...
  if (lang == "fr") {
    nms <- c(
      "Intégrité des berges", "Habitats", "Mammifères marins",
      "Sites d'intérêt culturels, patrimoniaux\net archéologiques"
    )
  } else if (lang == "en") {
    nms <- c(
      "Bank integrity", "Habitats", "Marine mammals",
  ...
  # Berge
  temp(
    "cumulative_effects_region_km2_berge",
  ...
  # Habitat
  temp(
    "cumulative_effects_region_km2_habitat",
  ...
  # Sites
  temp(
    "cumulative_effects_region_km2_site",
  ...
  i1 <- magick::image_read(here::here(output, "cumulative_effects_region_cea_berge.png"))
  i2 <- magick::image_read(here::here(output, "cumulative_effects_region_cea_habitat.png"))
  # i3 <- magick::image_read(glue("{output}cumulative_effects_region_cea_mammiferes_marins.png"))
  i4 <- magick::image_read(here::here(output, "cumulative_effects_region_cea_site.png"))

  l1 <- image_append(c(i1, i2))
  l2 <- image_append(c(i4))

  img <- image_append(c(l1, l2), stack = TRUE)
  magick::image_write(img, path = here::here(output, "cumulative_effects_region_cea_panel.png"), format = "png")
} # end function
