...
fig_cumulative_exposure_ceascf <- function(lang = "fr") {
  # ------------------
  global_parameters()
  ...
  # -----
  dat <- sf::st_read(here::here("data", "ceascf", "data-output", "cumulative_exposure.geojson"))
  ...
  i1 <- magick::image_read(here::here(output, "cumulative_exposure_berge.png"))
  i2 <- magick::image_read(here::here(output, "cumulative_exposure_habitat.png"))
  i4 <- magick::image_read(here::here(output, "cumulative_exposure_site.png"))

  l1 <- image_append(c(i1, i2))
  l2 <- image_append(c(i4))

  img <- image_append(c(l1, l2), stack = TRUE)
  magick::image_write(img, path = here::here(output, "cumulative_exposure_panel.png"), format = "png")
}
